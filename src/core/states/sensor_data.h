...
public: // methods
  ...
  Point2D move_origin(double d_x, double d_y, double d_angle) const {
    auto patched = ScanPoint2D{PointType::Polar, range(), angle() + d_angle,
                               is_occupied()};
  ...
  ScanPoint2D to_cartesian(double d_angle = 0, double d_range = 0) const {
    auto patched = ScanPoint2D{PointType::Polar, range() + d_range,
                               angle() + d_angle, is_occupied()};
  ...
  ScanPoint2D to_polar(double d_x = 0, double d_y = 0) const {
    auto patched =
        ScanPoint2D{PointType::Cartesian, x() + d_x, y() + d_y, is_occupied()};
...
std::ostream &operator<<(std::ostream &osm, const LaserScan2D &scan) {
  ...
  for (const auto &sp : scan.points()) {
    osm << sp.range() << " " << sp.angle() << " " << sp.is_occupied()
        << std::endl;
  }
  return osm;
}
