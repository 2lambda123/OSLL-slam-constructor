...
class SingleStateHypothesisLaserScanGridWorld : public LaserScanGridWorld {
public:
...
public:
  ...
  virtual void handle_observation(TransformedLaserScan &tr_scan) {
    ...
    tr_scan.quality =
        pose_delta ? _props.localized_scan_quality : _props.raw_scan_quality;

    scan_adder()->append_scan(map(), pose(), tr_scan.scan, tr_scan.quality,
                              _props.scan_margin);
  }
