...
public:
  GaussianPoseEnumerator(unsigned seed, double translation_dispersion,
      ...
      : _max_failed_attempts_per_shift{max_dispersion_failed_attempts},
        ...
        _pose_shift_rv{GaussianRV1D<Engine>{0, 0}, GaussianRV1D<Engine>{0, 0},
...
public:
  ...
  MonteCarloScanMatcher(std::shared_ptr<ScanProbabilityEstimator> estimator,
                        unsigned seed, double translation_dispersion,
                        double rotation_dispersion,
                        unsigned failed_attempts_per_dispersion,
                        unsigned total_attempts)
      : PoseEnumerationScanMatcher{
            estimator, std::make_shared<GaussianPoseEnumerator>(
  ...
  double process_scan(const TransformedLaserScan &raw_scan,
                      const RobotPose &init_pose, const GridMap &map,
    ...
    if (correlation > 0.9 and skipped_combo++ < 10 and !required_scan) {
      ...
      skipped_combo = 0;
      std::cout << "------------------------------" << std::endl
                << "scan proccesed" << std::endl;
      std::cout << total_scans++ << std::endl;
      return PoseEnumerationScanMatcher::process_scan(raw_scan, init_pose, map,
                                                      pose_delta);
    }
  }
...
private:
  ...
  std::vector<long> make_hist(const TransformedLaserScan &scan) {
    ...
    for (auto &p : scan.scan.points()) {
      histogram[int(column_amount * (p.range() - scan.range_min) /
                    range_size)]++;
    }
    return histogram;
  }
  ...
  std::vector<double> make_range_hist(const TransformedLaserScan &scan) {
    ...
    double column_part =
        (double)column_amount / (double)scan.scan.points().size();
    ...
    for (int i = 0; i < scan.scan.points().size(); i++) {
      if ((int)((double)i * column_part) <= column_amount) {
        ...
        sq_means[(int)((double)i * column_part)] +=
            scan.scan.points()[i].range() * scan.scan.points()[i].range();
      }
      // std::cout << i << " " << i / items_in_column << std::endl;
    }

    for (int i = 0; i < column_amount + 1; i++) {
      histogram[i] = sq_means[i] / (double)column_amount -
                     means[i] / (double)(column_amount * column_amount);
    }
  ...
  std::vector<double> make_TBM_hist(const TransformedLaserScan &scan) {
    ...
    for (int i = 0; i < points.size(); i++) {
      if ((int)((double)i * column_part) <= column_amount) {
        ...
        coherences[(int)((double)i * column_part)] *=
            MTBM(close_to_max * 0.9, close_to_min * 0.9, 0.1);
      }
    }
  ...
  template <typename T> void add_scan_to_buf(const std::vector<T> &raw_scan) {
  ...
  template <typename T>
  double calc_Pearson_correlation(const std::vector<T> &X,
    ...
    return (double)(n * sum_XY - sum_X * sum_Y) /
           //---------------------------------------------------------------------
           (sqrt((n * sq_sum_X - sum_X * sum_X) *
                 (n * sq_sum_Y - sum_Y * sum_Y)));
  }
  ...
  double calc_scan_information(const TransformedLaserScan &scan) {
    ...
    for (unsigned i = 0; i < points.size() - 3; i++) {
      ...
      point_is_in_front =
          (-M_PI_2 < points[i].angle()) && (points[i].angle() < M_PI_2);
      point_is_in_left = points[i].angle() < 0.0;
      score += sgn(points[i].range() - points[i + 3].range()) *
               (point_is_in_front ? 1 : -1) * (point_is_in_left ? 1 : -1);
      ;
    }
...
private:
  ...
  struct MTBM {
    ...
    double b;
    double ab;
    bool is_zero(double val) { return (val < 0.00001) && (-0.00001 < val); }
