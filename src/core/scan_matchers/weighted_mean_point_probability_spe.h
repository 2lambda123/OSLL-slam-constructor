...
public:
  WeightedMeanPointProbabilitySPE(OOPE oope, SPW spw, unsigned skip_rate = 0,
                                  double max_usable_range = -1)
      : ScanProbabilityEstimator{oope}, _spw{spw}, _pts_skip_rate{skip_rate},
        _pt_max_usable_range{max_usable_range} {}
  ...
  LaserScan2D filter_scan(const LaserScan2D &raw_scan, const RobotPose &pose,
    ...
    for (LaserScan2D::Points::size_type i = 0; i < raw_points.size(); ++i) {
      ...
      if (should_skip_point(sp, map, area_id)) {
        continue;
      }
  ...
  double estimate_scan_probability(const LaserScan2D &scan,
    ...
    for (LaserScan2D::Points::size_type i = 0; i < points.size(); ++i) {
      ...
      auto aoo_prob =
          occupancy_observation_probability(observation, obs_area, map);
...
protected:
  virtual bool should_skip_point(const ScanPoint2D &sp, const GridMap &map,
